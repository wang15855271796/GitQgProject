...
/**
 * Created by ${王涛} on 2019/11/9
 */
public class HomeFragments extends BaseFragment implements View.OnClickListener, BaseSliderView.OnSliderClickListener {
    Unbinder binder;
...
//    com.scwang.smartrefresh.layout.SmartRefreshLayout SmartRefreshLayout;
    ...
    //banner集合
    private List<HomeBaseModel.DataBean.BannerListBean> bannerList = new ArrayList<>();
    //公告
...
//                            skillAdvAdapter.notifyDataSetChanged();
                            ...
                            //banner
                            if (homeBaseModel.getData().getBannerList().size() > 0) {
                                banner.setVisibility(View.VISIBLE);
                                bannerList.clear();
                                bannerList.addAll(homeBaseModel.getData().getBannerList());
                                initBanner();
                            } else {
    ...
    private void initBanner() {
        ...
        if (bannerList.size() > 0) {
            for (int i = 0; i < bannerList.size(); i++) {
                ...
                defaultSliderView.image(bannerList.get(i).getBannerUrl());
                defaultSliderView.setScaleType(BaseSliderView.ScaleType.Fit);

                defaultSliderView.setOnSliderClickListener(this);
                defaultSliderView.bundle(new Bundle());
                defaultSliderView.getBundle().putString("banner_url", bannerList.get(i).getBannerDetailUrl());
                banner.addSlider(defaultSliderView);

            }
        }

        banner.setPresetTransformer(SliderLayout.Transformer.Default);
        banner.setPresetIndicator(SliderLayout.PresetIndicators.Center_Bottom);
        banner.setCustomAnimation(new DescriptionAnimation());
        banner.startAutoCycle(10000, 8000, true);
    }
