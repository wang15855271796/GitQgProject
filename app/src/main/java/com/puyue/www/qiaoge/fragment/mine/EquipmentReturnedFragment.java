...
public class EquipmentReturnedFragment extends BaseFragment {
    ...
    private int orderStatus = 10;//已归还
    ...
    public void findViewById(View view) {
        mPtr = ((PtrClassicFrameLayout) view.findViewById(R.id.ptr_equipment_returned));
        mRv = ((RecyclerView) view.findViewById(R.id.rv_equipment_returned));
        mIvNoData = ((ImageView) view.findViewById(R.id.iv_equipment_returned_no_data));
    }
        ...
        MyOrderListAPI.requestOrderList(getContext(), orderStatus, 1, 20,orderDeliveryType)
                ...
                .subscribe(new Subscriber<MyOrdersModel>() {
                    ...
                    public void onNext(MyOrdersModel myOrdersModel) {
                        ...
                        mModelEquipmentReturned = myOrdersModel;
                        if (mModelEquipmentReturned.success) {
                            updateList();
                        } else {
                            AppHelper.showMsg(getContext(), mModelEquipmentReturned.message);
                        }
                    }
                });
    }
    ...
    private void updateList() {
        if (mModelEquipmentReturned.data.list != null && mModelEquipmentReturned.data.list.size() > 0) {
            mRv.setVisibility(View.VISIBLE);
            mIvNoData.setVisibility(View.GONE);
            mListData.clear();
            mListData.addAll(mModelEquipmentReturned.data.list);
